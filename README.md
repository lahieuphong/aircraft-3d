...
Yêu cầu Node.js `>= 20.9.0`. Dự án ghim phiên bản Yarn bằng Corepack,
vì vậy không dùng npm hoặc tạo thêm `package-lock.json`.
...
`yarn build` tạo thư mục `dist/`. Lệnh `yarn start` chỉ dùng `serve` để xem thử chính thư mục này tại máy phát triển.
